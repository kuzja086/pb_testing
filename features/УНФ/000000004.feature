...
* Сценарий: Согласование с покупателем (Согласие)
	И я подключаю TestClient "Менеджеры" логин "Менеджеры" пароль ""
	И я закрыл все окна клиентского приложения
...
* Сценарий: Просмотр заказ-наряда и выполнение ремонта
	И я подключаю TestClient "Мастер" логин "Мастер" пароль ""
	И я закрыл все окна клиентского приложения
	И в поле с именем "Штрихкод" я ввожу текст "$Создание поступления техники для ремонта.СерийныйНомер$"
	Когда я нажимаю на кнопку 'Обработать штрихкод'
...
* Сценарий: Закрытие заказа регл. заданием
	...
	И я рисую рамку вокруг элемента панели разделов "Компания" в браузере
	| 'color'   | 'red'  |
	| 'size'    | '3'    |
	| 'timeout' | '5000' |
	Тогда открылось окно 'Больше возможностей: настройка программы'
	И в таблице "ДеревоРазделов" я разворачиваю текущую строку
	И я перехожу к закладке "Страница_РегламентныеОперации"
	И я нажимаю на гиперссылку "Регламентные операции"
	И у поля "Регламентные и фоновые задания" я нажимаю гиперссылку 'Регламентные и фоновые задания'
	...
	| 'Наименование' |
	| "(тл) Зкрытие заказ нарядов и заказов поставщиков"   |
	...
	| "(тл) Зкрытие заказ нарядов и заказов поставщиков"   | 'Задание выполнено' | '$ДатаОкончания$' |


@КодСценария=000000046
* Сценарий: Проверить начисление за ремонт
	И я подключаю TestClient "Администратор" логин "Администратор" пароль "1"
	И я закрыл все окна клиентского приложения
	И В командном интерфейсе я выбираю 'Зарплата' 'Отчеты'
	Тогда открылось окно 'Все отчеты'
	И я нажимаю на гиперссылку "Образец60e73152da8711e8ade000e04a6802bf"
	Тогда открылось окно 'Начисления и удержания'
	И я нажимаю на кнопку 'Сформировать'
	Дано Табличный документ "ОтчетТабличныйДокумент" равен макету "Начисления"
	И Я закрываю окно 'Начисления и удержания'
	Тогда открылось окно 'Все отчеты'
	И Я закрываю окно 'Все отчеты'
