...
Функциональность: Гарантийный ремонт

@КодНастройкиПроцесса=000000010
@ПолныйКодПроцесса=04.07
Сценарий: Гарантийный ремонт
...
* Сценарий: 4 Согласование с клиентом
	И я подключаю TestClient "Менеджеры" логин "Менеджеры" пароль ""
	И я закрыл все окна клиентского приложения
...
@КодСценария=000000013
* Сценарий: Перевод в гарантийный ремонт
	И я подключаю TestClient "Мастер" логин "Мастер" пароль ""
	И я закрыл все окна клиентского приложения
	*Мастер сч^итывает штрихкод на телефоне и видит сообщение об изменении статуса.
		И в поле с именем "Штрихкод" я ввожу текст "$Создание поступления техники для гарантийного ремонта.СерийныйНомер$"
...
@КодСценария=000000026
* Сценарий: 6. Просмотр заказ-наряда и выполнение ремонта
	И я подключаю TestClient "Мастер" логин "Мастер" пароль ""
	И я закрыл все окна клиентского приложения
...
* Сценарий: Возврат техники
	...
	*Открываем документ приема в ремонт, заполняем срок гарантии, указываем что техника выдана покупателю. Также необходимо распечатать акт приема передачи и гаратнтийный талон и подписать с клиентом.
		...
		Если переменная "Отказ" имеет значение "Ложь" Тогда
