...
#СуммаДокумента=2,00
#Цена=1,00
#ШтрихкодМатериала=2000000004082
#ШтрихкодРаботы=2000000004587
...
* Сценарий: Создание заказа поставщику
	...
	*И Проверка строки заказа поставщику [СерийныйНомер] [Количество] [Цена] [Сумма]
		...
		И я запоминаю значение текущего поля как "$Создание заказа поставщику.СерийныйНомер$"
		И в таблице "Запасы" яперехожу к следующей ячейке
		И в таблице "Запасы" поле "Серийный номер" имеет значение "$Создание заказа поставщику.СерийныйНомер$"
	...
	*И Проверка строки заказа поставщику [СерийныйНомерОтказ] [Количество] [Цена] [Сумма]
		И я активизирую поле "Характеристика"
		И в таблице текущее поле заполнено
		И я запоминаю значение текущего поля как "$Создание заказа поставщику.СерийныйНомерОтказ$"
		И в таблице "Запасы" яперехожу к следующей ячейке
		И в таблице "Запасы" поле "Серийный номер" имеет значение "$Создание заказа поставщику.СерийныйНомерОтказ$"
		И в таблице "Запасы" поле "Количество" имеет значение "1,000"
		И в таблице "Запасы" поле "Цена" имеет значение "1,00"
		И в таблице "Запасы" поле "Сумма" имеет значение "1,00"
...
* Сценарий: Забор техники
	...
	И я закрыл все окна клиентского приложения
	И в поле с именем "Штрихкод" я ввожу текст "$Создание заказа поставщику.СерийныйНомер$"
...
* Сценарий: Поступление техники
	...
	Если в таблице "Запасы" есть строка Тогда
			| 'Серийные номера' |
			| '$Создание заказа поставщику.СерийныйНомер$' |
			...
			Тогда элемент формы с именем "тлСерийныйНомер" стал равен "$Создание заказа поставщику.СерийныйНомер$"
...
* Сценарий: Добавление работ
	И я подключаю TestClient "Мастер" логин "Мастер" пароль ""
	И я закрыл все окна клиентского приложения
	Когда в поле 'Штрихкод' я ввожу текст "$Создание заказа поставщику.СерийныйНомер$"
	И я нажимаю на кнопку 'Обработать штрихкод'
	Тогда открылось окно 'Заказ-наряд * от *'
	И я перехожу к закладке с именем "ЗНГруппаРаботы"
	И в поле 'Тл штрихкод' я ввожу текст "2000000004587"
	И я нажимаю на кнопку 'Тл добавить штрихкод'
	И в поле 'Тл штрихкод' я ввожу текст "2000000004082"
	И я нажимаю на кнопку 'Тл добавить штрихкод'
	И в поле 'Тл штрихкод' я ввожу текст "$Создание заказа поставщику.СерийныйНомер$"
	И я нажимаю на кнопку 'Тл добавить штрихкод'
	И я нажимаю на кнопку 'тлИзменить статус'
	Затем я жду, что в сообщениях пользователю будет подстрока "установлен статус" в течение 3 секунд
	И Я закрываю окно 'Заказ-наряд * от *'
